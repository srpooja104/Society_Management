    ...
    <div className="flex bg-[#FDF5FB]">
      ...
      <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100">
        <div className="container mx-auto py-8">
            ...
            <div className="flex justify-between items-center">
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                  {[
                    ...
                    <div key={index} className="bg-white rounded-lg p-3 shadow-md px-8">
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="text-[#000000] text-md">{stat.title}</p>
                          <p className={`text-2xl font-bold mt-1 ${stat.color}`}>{stat.amount}</p> {/* Color applied here */}
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <button className='text-white font-bold px-6 py-4 rounded-t-lg' 
             style={{
              background: "linear-gradient(to right, #FE512E, #F09619)",
              transition: "background 0.3s ease",
        
          }}>Maintenance</button>
          <button className='bg-white px-6 py-4 rounded-t-lg border-2 border-white border-b-orange-500 font-bold'>Other Income</button>
                  ...
                  {maintenanceData.map((item) => (
                    <tr key={item.id} >
                      ...
                      <td className=" whitespace-nowrap w-32"> 
                          <span
                            className={` py-2 inline-flex text-md leading-5 font-semibold rounded-full items-center justify-center gap-2 ${
                              item.penalty === 0
                      ...
                      <td className="px-6 py-2 whitespace-nowrap">
                          <span className={`w-32 py-2 inline-flex text-md leading-5 font-semibold rounded-full items-center justify-center ${
                            ...
                            {item.paymentStatus === 'Pending' ? (
                              <>
                                <HiClock className="mr-2 text-lg" /> {/* Larger icon for Pending status */}
                                {item.paymentStatus}
                              </>
                            ) : (
                              <>
                                <HiBadgeCheck className="mr-2 text-lg" /> {/* Larger icon for completed status */}
                                {item.paymentStatus}
                              </>
                            )}
                          </span>
                        ...
                        <span className={`w-32 py-2  inline-flex text-md leading-5 font-semibold rounded-full items-center justify-center text-center ${
                          ...
                          {item.paymentMethod === 'Online' ? (
                            <>
                              < BiSolidCreditCardAlt className="mr-2 text-lg" /> {/* Icon for Online */}
                              {item.paymentMethod}
                            </>
                          ) : (
                            <>
                              <HiCash className="mr-2 text-lg" /> {/* Icon for Cash */}
                              {item.paymentMethod}
                            </>
                          )}
                        </span>
